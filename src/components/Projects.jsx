    ...
    <section id="projects" className="snap-section">
      <div className="section-inner">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
          {projects.map((p) => (
            <div key={p.key} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '2px', padding: '1.25rem', transition: 'border-color 0.2s', cursor: 'default' }}
              onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--accent)'}
              onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                <span style={{ fontFamily: 'var(--mono)', fontSize: '14px', fontWeight: 700, color: 'var(--fg)' }}>{t(`projects.items.${p.key}.name`)}</span>
              ...
              <p style={{ fontSize: '12px', color: 'var(--fg2)', lineHeight: 1.6, marginBottom: '1rem', fontWeight: 300 }}>{t(`projects.items.${p.key}.desc`)}</p>
